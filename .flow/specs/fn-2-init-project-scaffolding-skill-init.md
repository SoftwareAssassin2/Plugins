...
- **Git-commit policy (inline `_CLAUDE.md` directive, R21):** no commits to `main` without explicit user instruction (ask first if a changeset seems commit-worthy on `main`); commit freely on non-default branches; always `push` after any commit. Mirrors this repo's own operating guidance.
...
- **R21:** `_CLAUDE.md` carries an inline **git-commit policy** directive: **never commit to the `main` (default) branch without explicit user instruction** — if Claude judges a changeset is commit-worthy while on `main`, it must **ask the user for permission first**; on any **non-default branch it may commit freely**; and **every commit, on any branch, is immediately followed by a `push`**.
...
| R21 | Inline _CLAUDE.md git-commit policy: no commit to main without explicit OK (ask first), free commits on other branches, always push after commit | fn-2….2 | — |
