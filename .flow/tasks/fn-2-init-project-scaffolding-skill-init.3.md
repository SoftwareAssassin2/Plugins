...
**Size:** M
**Files:** `plugins/init-project/templates/docs/config-management.md`, `plugins/init-project/templates/config.json`, `plugins/init-project/templates/config.deploy.json`
...
**Required:** (raw `config-management.md` source moves `plugins/project-init/`→`plugins/init-project/` via fn-2….1; path below is post-rename)
- `plugins/init-project/config-management.md:1-16,84-115` — portable kernel (§1, §5 password alphabet, §6 12-factor)
- `plugins/init-project/config-management.md:40-61` — owner/migrator/api role pattern to generalize into the `postgres` systems entry
...
- Tests: bash plugins/init-project/tests/scaffold_test.sh (44 passed, 0 failed)
- PRs: