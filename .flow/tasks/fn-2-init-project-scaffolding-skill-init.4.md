...
## Acceptance
- [ ] devcontainer.json: base `devcontainers/dotnet:1-9.0` + pinned node/aws-cli/azure-cli/github-cli (official — `gh` is the first-party feature; GitLab `glab` has none, so it installs via setup.sh) + gcloud (community `:1.0.1`)
- [ ] setup.sh installs Angular CLI, DuckDB, `acli`, `glab` (GitLab CLI, version-pinned official binary), Claude Code (install.sh), Codex CLI
