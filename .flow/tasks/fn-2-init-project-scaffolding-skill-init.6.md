...
**Size:** M
**Files:** `src/init-project/templates/system.sh`, `src/init-project/templates/src/system-cli/{help,build-config,up,down,migrate,status}.sh`, `src/init-project/templates/tests/system-cli/` (generated-project shell test harness), `src/init-project/tests/` (skill-package kcov tests for scaffold.sh)

## Approach (carry forward source conventions)
- **Dispatcher path (note from fn-2….1 review):** the generated `system.sh` lives at the **repo root**, so `script_dir`=repo root and `$script_dir/src/system-cli/<sub>.sh` resolves correctly. (The raw source copy at `src/init-project/system-cli/system.sh` mis-resolves from its own location — author the template for the root location; don't copy the source's `$script_dir` assumption blindly.)
...
**Required:** (dispatcher source moves `src/project-init/system-cli/`→`src/init-project/system-cli/` via fn-2….1; paths below post-rename)
- `src/init-project/system-cli/system.sh:1-38`, `help.sh:1-43` — dispatcher + Description-scraping source
- `src/init-project/templates/docs/tdd.md` (fn-2….8) — coverage standard + kcov tooling
- `src/init-project/scaffold.sh` (fn-2….1) — the other script under test
