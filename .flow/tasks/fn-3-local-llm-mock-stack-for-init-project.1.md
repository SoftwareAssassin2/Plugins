...
- Split evidence: the **unit gate** is generated-file + mocked-shell (no daemon); an **optional Docker-enabled integration job** runs `--profile ai-mock` (no model) and asserts both surfaces **plus a non-streaming tool-use round-trip** (deterministic gateway-regression signal); real-inference `--profile ai` smoke is MANUAL.
...
- [ ] optional Docker-enabled integration job: `--profile ai-mock` serves canned responses on BOTH `/v1/chat/completions` + `/v1/messages`, INCLUDING a non-streaming tool-use round-trip (distinct from the no-daemon unit gate) (R3, R10)
- [ ] Ollama image pinned to a version tag (not `latest`); CPU-only default, no GPU reservation in the committed compose (R14, R15)
- [ ] embeddings (R12): `config.yaml.template` carries the conditional `local-embed` → `ollama/@@LLM_EMBED_MODEL@@` entry (OpenAI `/v1/embeddings`); `_pull-model.sh` pulls `LLM_EMBED_MODEL` too when set; `ai-mock` has no embeddings entry (R12)
