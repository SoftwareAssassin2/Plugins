...
**Contracts (epic §Canonical Contracts is authoritative):** C1 (mode), C3 (`plugin_state_dir` / toggle), C4 (probe is advisory, never a send gate), C8 (debounce markers + sanitization), C9 (constants incl. hook timeout).

- `hooks/hooks.json`: a `Stop` hook (**no matcher**), exec form, invoking the hook script with the hook-config `timeout` field = **10** seconds per C9 (the hooks.json field — NOT the external `timeout`/`gtimeout` binary, which macOS lacks; 10s comfortably covers the synchronous preflight path, and playback is detached so speech duration never counts against it). Mirror the header/contract style of `plugins/init-project/templates/.claude/hooks/claude-md-reminder.sh` (Description header, **always exit 0, never block**).
...
- `stop-speak.sh` sources `bin/speak` for its helpers. Because `bin/speak` enables `set -euo pipefail` **only when executed** (source-guarded), sourcing does NOT leak strict mode into the hook; the hook checks helper return codes explicitly and **always exits 0**. It runs as one **ordered algorithm**:
  ...
  5. **Preflight synchronously** via the shared helpers (`deps_missing`, `speak_mode`, port-validate, `listener_reachable`). **Reachability is checked ONLY when `speak_mode=forward`; in local mode preflight only the local dep `say` + `jq` (base64 is NOT a local dep — wire-transport only) and never emit a "start `speak --serve`" notice.** Two distinct outcomes:
     ...
     Notices are **self-contained** (no hard README dependency; `.7` verifies agreement). Debounce markers are keyed exactly per **C8** (epic §Canonical Contracts — path, reason set incl. `invalid-config`, sanitization, 7-day prune on hook entry). The hook's forward notify decision uses the **single fast `listener_reachable` probe** (advisory, per C4) — not the retry/backoff path.
...
- `speak on` / `speak off`: write the single global flag file **`<plugin_state_dir>/auto-speak` containing exactly `on\n` or `off\n`, atomically (temp + rename)**, with `plugin_state_dir` resolved per **C3** (NO HOME fallback — the hook reads the same dir; NOT `${CLAUDE_PLUGIN_ROOT}`, which is ephemeral). Any other/absent/unreadable content reads as OFF. Outside Claude with neither var set, `speak on/off` reports "toggle unavailable" per C3 (it does NOT write a HOME dir the hook won't read). (The diagnostic surface — `speak doctor`, with `speak status` as its alias — ships in `.5`.)
...
- [ ] Transcript schema: implement against the assumed schema; confirm against a real transcript captured at Stop time (manual evidence), specifically that (a) the FINAL line is the assistant message (no trailing non-assistant records) and (b) a complete response is ONE record (not split across multiple assistant records) — either failing silently mutes/truncates auto-speak, so verify, don't assume. If unavailable in the impl env, the **explicit fallback** is — ship against the assumed schema, `.6` includes fixtures documenting the assumption, and a manual verification note is required before auto-speak is considered complete. Fixture capture + tests owned by `.6`
...
## Done summary
Wired the speak plugin's Stop hook (hooks/hooks.json + hooks/stop-speak.sh, always-exit-0, timeout 10 per C9), the atomic `speak on/off` auto-speak toggle under plugin_state_dir (C3), the shared deps_missing helper (nc capability per C6), and once-per-session+reason debounced systemMessage notices (C8, jq-free static emitter on the no-jq path). Confirmed the transcript JSONL schema against REAL Stop-time captures (probe hook, Claude Code 2.1.203): both assumed properties were false — the final line is a bookkeeping trailer, and one response spans multiple records sharing one message.id — so the sourceable extract_last in bin/speak implements the verified real schema (last main-chain conversation-message record must be assistant; join text parts of its message.id cluster; never skips past a user record). Codex impl-review: SHIP (first pass).
## Evidence
- Commits: c8e26c0d3e1164a810e70146ddc7cc6997a21216
- Tests: shellcheck -x plugins/speak/hooks/stop-speak.sh plugins/speak/bin/speak (clean), bash -n on both scripts, extract_last vs REAL Stop-time transcript: 2885 chars == hook stdin last_assistant_message (exact); user-tail/missing/torn-line edges all silent, toggle: on/off write on\n/off\n atomically; ON/corrupt/absent/no-vars all read OFF; neither var -> 'toggle unavailable' rc=1; no strict-mode leak when sourced, hook E2E (synthesized real-schema stdin): toggle-off silent; local dispatch detached to stubbed say (temp cleaned by child); stop_hook_active guard on jq path; no-jq static-JSON notice once + whitespace-tolerant grep guard (0 markers); forward probe-fail -> notice once + send STILL dispatched (child log shows 3-attempt retry); Apple-nc missing EOF-capability -> missing-nc notice, no dispatch; hostile SPEAK_PORT (quote+newline) -> valid JSON invalid-port notice, no dispatch; missing say -> missing-say notice; invalid SPEAK_MODE -> invalid-config; distinct reason markers coexist; >7-day marker pruned on entry; non-assistant tail silent, LIVE E2E: real claude -p session with Stop hook wired -> real hook stdin drove stop-speak.sh -> stubbed say received the exact assistant text, stdout clean, transcript-schema verification (epic Open Question): probe Stop hook captured hook stdin + transcript at Stop time on Claude Code 2.1.203 — assumption (a) FALSE: final line is a bookkeeping trailer (attachment/system/ai-title), not the assistant record; assumption (b) FALSE: one response spans multiple records sharing message.id (thinking+text). extract_last implements the verified real schema; fixtures for .6 preserved at scratchpad stopprobe/transcript2.jsonl semantics documented in extract_last header
- PRs: