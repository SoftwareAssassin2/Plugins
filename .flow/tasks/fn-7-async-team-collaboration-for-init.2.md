...
- [ ] Routing is latest-turn-wins: a thread with turns `awaiting-assignee → awaiting-asker → resolved` does NOT alert (verified by .4's behavioral fixture); identity matched by `git-email` against `team.md`, then by the row's **stored whole-email-slug handle** (not local-part-only, not re-derived) against the thread-header asker/assignee
...
## Done summary
Implemented the read-only SessionStart `collaboration-inbox.sh` hook (git-identity resolution at project root, three identity cases, latest-turn-wins streaming inbox parse, JSON additionalContext envelope with jq-absent raw-text fallback, guarded freshness) and wired `hooks.SessionStart` into the scaffolded `.claude/settings.json` as a sibling of the existing Stop hook. Codex impl-review: SHIP (R2/R3/R8/R9/R10 met); scaffold_test 355/0.
## Evidence
- Commits: c3733694a71de11b97cf1850df903d743e9b9a10
- Tests: shellcheck plugins/init-project/templates/.claude/hooks/collaboration-inbox.sh, bash -n plugins/init-project/templates/.claude/hooks/collaboration-inbox.sh, jq -e settings.json validity, behavioral matrix: case A/B/C/no-pending/no-upstream/jq-absent fallback, bash plugins/init-project/tests/scaffold_test.sh (355 passed, 0 failed)
- PRs: