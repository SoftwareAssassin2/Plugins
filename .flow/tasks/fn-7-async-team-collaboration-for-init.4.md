...
- **Behavioral hook block (proves R8/R10):** isolate git config first (`HOME=<tmp>`, `GIT_CONFIG_GLOBAL=/dev/null`, neutralize system config) so identity reads ONLY what the test sets (else the CI machine's global identity leaks in and the "no identity" case is flaky). **`git init "$WORK/demo-app"`** and set/omit identity repo-locally via `git -C "$WORK/demo-app" config user.email …` per case (the hook reads at the project root). Write fixture `docs/team.md` (the fixed table — columns `handle | name | git-email | computer-name | reports-to`, header + `|---|` separator + data rows, as authored in `collaboration.md`/`team.md`) and `docs/collaboration/*.md`, then run the hook. **Fixture handles MUST be whole-git-email slugs** as finalized in `collaboration.md` (lowercase the entire email; non-`[a-z0-9]` runs → single `-`; trim) — NOT local-part-only: a fixture `git-email` of `bob.smith@x.com` has handle `bob-smith-x-com` (and `inbox` file `docs/collaboration/bob-smith-x-com.md`), never `bob-smith`. Thread headers use `## thread:<id> | asker:<handle> | assignee:<handle> | subject:<text>`; turn headers use `### turn <n> | author:<handle> | status:<enum> | <iso-ts>`; `status` ∈ `awaiting-assignee | awaiting-asker | resolved` — match these exact shapes verbatim. <!-- Updated by plan-sync: fn-7-async-team-collaboration-for-init.1 finalized whole-email-slug handles + exact team.md/thread/turn/status shapes --> Include a fixture thread whose **subject/body contains quotes + a newline** to prove `additionalContext` is JSON-escaped (output still `jq`-parses). **Split assertions by output kind:** JSON cases (pending thread present) pipe stdout to `jq` and assert `hookSpecificOutput.hookEventName=="SessionStart"` + the expected `additionalContext` (latest-turn routing — `awaiting-assignee→awaiting-asker→resolved` does NOT alert; asker routing — own `awaiting-asker` surfaces; freshness text present); **silent cases** (no `user.email` even with `user.name` set; in-team but no pending; no `docs/collaboration/`) assert **empty stdout + exit 0** (do NOT pipe to jq); the **identity-not-in-team** case asserts the register advisory via `jq` (same SessionStart `additionalContext` envelope); and a no-upstream fixture asserts no `@{u}` error.
...
- [ ] Fixtures encode the finalized contract: `team.md` columns `handle | name | git-email | computer-name | reports-to`; **handles are whole-git-email slugs** (e.g. `bob.smith@x.com` → `bob-smith-x-com`), NOT local-part-only; thread/turn headers + `awaiting-assignee | awaiting-asker | resolved` status vocabulary match the authored shapes verbatim
