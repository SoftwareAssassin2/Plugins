...
check "setup.sh best-effort enables 9 marketplace plugins" "for p in dick handoff tdd ubiquitous-language domain-modeling grilling grill-with-docs preferred-browser-automation-plugin preferred-ralph-loops-plugin; do grep -q \"\$p\" \"$SUP\" || exit 1; done"
check "setup.sh no longer enables grill-me (replaced by grill-with-docs)" "! grep -q 'grill-me' \"$SUP\""
