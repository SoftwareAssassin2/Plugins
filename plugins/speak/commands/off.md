...
- **Success** (it prints `speak: auto-speak is now off ...`): confirm to the user that auto-speak is OFF — responses are no longer spoken automatically (they can still use `/speak:now` on demand). Note: this does not interrupt any speech already playing.
