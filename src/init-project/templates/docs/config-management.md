...
configurable but validated for shape (port range, host/realm regex). Fields marked
**fixed** are scaffold constants, not in `config.json`.

### `postgres` (`systems[]` entry `postgres`)

| `config.json` field | Kind | Validator | Generated `.env` var | Consumer |
|---|---|---|---|---|
| `host` | structural | host regex | — (host-side `psql`/`pg_isready`) | devcontainer tooling |
| `container_host` | structural | container-name regex | `POSTGRES_HOST` | compose, `Api`, `migrator` conn-strings |
...
Generated `.env`: `src/postgres/.env`. The `Api` and migrator connection strings
are assembled by `build-config` from `container_host`/`container_port` + the
relevant role + password + the fixed `platform` db name.

### `keycloak` (`systems[]` entry `keycloak`)

| `config.json` field | Kind | Validator | Generated `.env` var | Consumer |
|---|---|---|---|---|
| `host` | structural | host regex | — (host-side tooling) | devcontainer tooling |
| `container_host` | structural | container-name regex | `KEYCLOAK_HOST` | compose, in-network references |
...
| `public_url` | structural | URL | `KEYCLOAK_PUBLIC_URL` | `Api` issuer/authority, SPA public config |
...
|---|---|---|---|---|
| `realm_url` | public, structural | URL | `src/<SPA>/public/config.json` `realmUrl` | SPA OIDC config (browser) |
| `public_client_id` | public, structural | client-id regex | `src/<SPA>/public/config.json` `clientId` | SPA OIDC config (browser) |

### `Api` (`systems[]` entry `Api`)

| `config.json` field | Kind | Validator | Generated `.env` var | Consumer |
|---|---|---|---|---|
| `host` | structural | host regex | `API_HOST` | `Api` listener |
| `port` | structural | port `1..65535` | `API_PORT` | `Api` listener |

The `Api` also reads the postgres `api` connection string and the keycloak
issuer/client values (assembled by `build-config` into `src/Api/.env`) for JWT
validation and the least-privilege DB connection.
