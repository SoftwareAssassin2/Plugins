...
describe('MarketingSite startup config initializer', () => {
  ...
  it('fetches /config.json during app initialization and populates AppConfigService', async () => {
    ...
    req.flush({ realmUrl: 'http://127.0.0.1:8080/realms/demo', clientId: 'marketingsite' });
    await ready;
    expect(TestBed.inject(AppConfigService).get().clientId).toBe('marketingsite');
  });
});
